...
function publicNote(c){
 ...
 if(c.id==='central-bendigo') return 'Published overnight hours conflict. Sunday overnight on-call is listed as unavailable. Call to confirm care and location.';
 if(c.id==='gippsland-warragul') return 'The website lists different weekend start times. Call to confirm, particularly on Friday evenings.';
 if(c.id==='geelong-emergency') return 'Overnight hours differ between published sources. Call to confirm before travelling.';
 if(c.id==='advanced-kew') return 'Call to confirm the entrance and availability before travelling.';
 if(c.verification_status==='needs_review') return 'The full after-hours schedule needs confirmation. Call before travelling.';
 return '';
}
function card(c){
